...
router.post("/request", AuthMiddleware, async (req, res) => {
    ...
    if (senderId === receiverId) {
        return res
            .status(400)
            .json({ message: "لا يمكنك إرسال طلب صداقة لنفسك." });
    }
    const receiver = await User.findById(receiverId);
    if (!receiver) {
        return res
            .status(404)
            .json({ message: "المستخدم المُستَقبِل غير موجود." });
    }
  ...
  async (req, res) => {
    ...
    try {
        ...
        if (pendingRequest) {
            ...
            // =========================================================================
            const direction = pendingRequest.sender.toString() === currentUserId 
...
router.get("/friends", AuthMiddleware, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: "فشل في جلب قائمة الأصدقاء.", error: error.message });
    }
});
