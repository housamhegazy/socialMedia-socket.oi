...
// 🔥 2. تهيئة Mutation لوضع علامة القراءة
  const [markRead, { isLoading: isMarking }] = useMarkNotificationAsReadMutation();
              ...
              <Box key={notif._id}>
                <ListItem
                  ...
                  sx={{
                    py: 2,
                    cursor: "pointer",
                    backgroundColor: !notif.isRead ? theme.palette.action.hover : theme.palette.background.paper,
                ...
                >
                  ...
                  {/* 1. أفاتار المرسل */}
                  <ListItemAvatar>
                    <Avatar src={notif.sender?.avatar} alt={notif.sender?.name} />
                  </ListItemAvatar>
                  ...
                  <ListItemText
                    ...
                    primary={
                      <Box>
                        <Typography component="span" variant="body1" fontWeight={!notif.isRead ? "bold" : "normal"}>
                          <span style={{ color: theme.palette.primary.main }}>{notif.sender?.name}</span> 
                          {notif.type === "reply" && " replied to your comment "}
                          {notif.type === "like" && " liked your post "}
                          {notif.type === "comment" && ` commented on your post "${notif.post?.text ? notif.post.text.substring(0, 30) + '...' : '...'}" `}
                  ...
                  {!notif.isRead && (
                    <Button
                      size="small"
                      onClick={(e) => {
                        e.stopPropagation(); // منع النقر على الـ ListItem
